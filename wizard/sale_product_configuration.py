        ...
        for conf in self:
            ...
            for product_id in product_ids:
                for indx, ws in enumerate(wh_ids):
                    # Buscar ubicaciones internas dentro del almacén que estén marcadas para ser consideradas por el cotizador.
                    allowed_locations = self.env['stock.location'].search([
                        ('id', 'child_of', ws.view_location_id.id),
                        ('usage', '=', 'internal'),
                        ('x_consider_for_cosal_quoter', '=', True)
                    ])

                    # Si no hay ubicaciones permitidas en este almacén, saltar al siguiente.
                    if not allowed_locations:
                        continue

                    qty_available = product_id.with_context(location=allowed_locations.ids).qty_available
